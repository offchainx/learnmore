...
export const ReportDetailsDrawer: React.FC<ReportDetailsDrawerProps> = ({ isOpen, onClose, report }) => {
  ...
  const getOptionIcon = (optionId: string) => {
      if (optionId === systemAnswerId) return <span className="material-icons-outlined text-green-500 text-lg">check</span>;
      if (optionId === userAnswerId) return <span className="material-icons-outlined text-red-500 text-lg">close</span>;
      return null;
  };
    ...
    <div className={`fixed inset-y-0 right-0 w-full sm:w-[500px] bg-white dark:bg-[#0B0E14] drawer-glass shadow-2xl transform transition-transform duration-300 ease-in-out z-50 flex flex-col border-l border-white/10 overflow-hidden ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      ...
      <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-white/10 relative z-10 bg-white/50 dark:bg-black/20 backdrop-blur-md">
        <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
          <span className="material-icons-outlined text-purple-500">analytics</span>
          Report Details
        </h2>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 relative z-10">
        <div className="flex items-start justify-between">
          ...
          <div className="flex flex-col items-end">
            <span className="px-3 py-1 text-xs font-semibold rounded-full bg-red-100 dark:bg-red-500/10 text-red-800 dark:text-red-400 border border-red-200 dark:border-red-500/20 mb-1">
              {report.issueType}
            </span>
        ...
        <div>
          <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-3 flex items-center gap-2">
            <span className="material-icons-outlined text-primary text-base">help_outline</span>
            Question Content
          </h4>
