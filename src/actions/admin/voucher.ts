...
export async function createVoucherCodeAction(input: CreateVoucherInput): Promise<VoucherActionResult> {
...
export async function toggleVoucherStatusAction(voucherId: string, isActive: boolean): Promise<VoucherActionResult> {
  const admin = await ensureAdmin();
  if (!admin) {
    return {
      ok: false,
