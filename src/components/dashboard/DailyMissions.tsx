      ...
      <div className="relative z-10 flex h-full flex-col p-5">
        ...
        <div className="space-y-3" onWheel={handleWheel}>
          {visibleTasks.map((task, index) => {
            ...
            let statusColor =
              'text-blue-700 bg-blue-50 border-blue-200 dark:text-blue-400 dark:bg-blue-400/10 dark:border-blue-400/20'
            if (task.isClaimed) {
              statusColor =
                'text-slate-600 bg-slate-100 border-slate-200 dark:text-slate-400 dark:bg-slate-400/10 dark:border-slate-400/20'
            } else if (isCompleted) {
              statusColor =
            ...
            return (
              <div
                ...
                className={`group flex items-center justify-between rounded-[22px] border px-4 py-3.5 shadow-none transition-all duration-300 animate-in fade-in slide-in-from-right-4 fill-mode-both ${
                  task.isClaimed
                    ...
                    : isInteractive
                      ? 'cursor-pointer border-borderTone bg-surface hover:border-cyan-300/60 hover:bg-surface-subtle dark:border-white/10 dark:bg-white/[0.04] dark:hover:bg-white/[0.08]'
              ...
              >
                <div className="flex flex-1 items-center gap-4">
                  ...
                  >
                    <div
                      className={`h-2 w-2 rounded-full bg-current ${task.isClaimed ? 'text-slate-400 dark:text-slate-500' : 'text-cyan-500 dark:text-cyan-300'}`}
                    />
                  </div>
                  <div className="flex-1">
                    <div
                      className={`text-[15px] font-bold transition-colors ${task.isClaimed ? 'text-text-secondary dark:text-slate-400' : 'text-text-primary group-hover:text-cyan-700 dark:text-white dark:group-hover:text-cyan-100'}`}
                    >
                      {task.title}
                    </div>
                    <div className="mt-1 flex items-center gap-3">
                      ...
                      <div className="h-1.5 max-w-[100px] flex-1 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
                        <div
                          className="h-full rounded-full bg-cyan-500 transition-all duration-500 dark:bg-cyan-300"
