...
export function DashboardStreakLayoutInspector({
  visible,
  selected,
  preset,
  saving,
  ...
  saving: boolean
  onPresetChange: (preset: DashboardStreakLayoutPreset) => void
  onSave: () => void
}) {
  if (!visible) {
    return null
      ...
      {!selected ? (
        ...
        <div className="mt-4 space-y-4">
          ...
          <section className="space-y-3 rounded-[18px] bg-[#fbf6ee] p-3">
            ...
            <RangeField
              label="X"
              value={preset.campfireFrame.x}
              min={120}
              max={420}
              onChange={(next) =>
                onPresetChange({
                  ...preset,
                  campfireFrame: { ...preset.campfireFrame, x: next },
                })
              }
            />
            <RangeField
              label="Y"
              value={preset.campfireFrame.y}
              min={-80}
              max={80}
              onChange={(next) =>
                onPresetChange({
                  ...preset,
                  campfireFrame: { ...preset.campfireFrame, y: next },
                })
              }
            />
            <RangeField
              label="宽度"
              value={preset.campfireFrame.width}
              min={80}
              max={220}
              onChange={(next) =>
                onPresetChange({
                  ...preset,
                  campfireFrame: { ...preset.campfireFrame, width: next },
                })
              }
            />
            <RangeField
              label="高度"
              value={preset.campfireFrame.height}
              min={70}
              max={220}
              onChange={(next) =>
                onPresetChange({
                  ...preset,
                  campfireFrame: { ...preset.campfireFrame, height: next },
                })
              }
            />
          </section>
