...
export function DashboardPathLayoutInspector({
  visible,
  selected,
  preset,
  saving,
  ...
  saving: boolean
  onPresetChange: (preset: DashboardPathLayoutPreset) => void
  onSave: () => void
}) {
  if (!visible) {
    return null
      ...
      {!selected ? (
        ...
        <div className="mt-4 space-y-4">
          ...
          <section className="space-y-3 rounded-[18px] bg-[#fbf6ee] p-3">
            ...
            <RangeField
              label="Y"
              value={preset.backgroundOffset.y}
              min={-120}
              max={120}
              onChange={(next) =>
                onPresetChange({
                  ...preset,
                  backgroundOffset: { y: next },
                })
              }
            />
          </section>
