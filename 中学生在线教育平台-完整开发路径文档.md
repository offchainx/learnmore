# 中学生在线教育平台 - 完整开发路径文档

## 📋 目录

- [1. 项目概述](#1-项目概述)
- [2. 产品功能拆解](#2-产品功能拆解)
- [3. 技术架构设计](#3-技术架构设计)
- [4. 数据库设计](#4-数据库设计)
- [5. API 接口设计](#5-api-接口设计)
- [6. 开发路线图](#6-开发路线图)
- [7. 技术难点与解决方案](#7-技术难点与解决方案)
- [8. 性能与扩展性](#8-性能与扩展性)
- [9. 成本预算](#9-成本预算)
- [10. 质量保证](#10-质量保证)

---

## 1. 项目概述

### 1.1 项目背景

中学生在线教育平台是一个面向初中学生的综合性学习平台，提供课程学习、题库练习、学习统计、社区讨论等功能，帮助学生提升学习效率和成绩。

### 1.2 产品定位

- **目标用户**：初中7-9年级学生（12-16岁）
- **核心价值**：个性化学习、数据驱动、游戏化激励
- **产品形态**：Web 应用（响应式设计，支持移动端）

### 1.3 核心功能

1. ✅ 用户系统（注册、登录、权限管理）
2. ✅ 课程学习（6大学科，视频+文档）
3. ✅ 题库练习（智能组卷、自动批改）
4. ✅ 学习统计（多维度数据分析）
5. ✅ 讨论社区（学习交流、问题解答）
6. ✅ 成长系统（等级、成就、排行榜）

### 1.4 设计稿

Figma 设计稿已完成，包含：

- 首页仪表盘
- 学习模块（6大学科）
- 题库练习
- 讨论区
- 成长中心
- 用户中心

设计链接：https://www.figma.com/make/xOrPWIHw7gqEuKx2oBkayC

---

## 2. 产品功能拆解

### 2.1 用户系统

#### 功能列表

- **注册登录**
  - 手机号/邮箱注册
  - 验证码登录
  - 密码登录
  - 第三方登录（微信、QQ）可选

- **用户信息管理**
  - 基本资料（姓名、年级、班级）
  - 头像上传
  - 联系方式
  - 学习偏好设置

- **权限管理**
  - 学生角色
  - 教师角色（V2.0）
  - 管理员角色
  - 家长角色（V2.0）

#### 数据表设计

```sql
-- 用户表
CREATE TABLE users (
  id SERIAL PRIMARY KEY,
  username VARCHAR(50) UNIQUE NOT NULL,
  email VARCHAR(100) UNIQUE,
  phone VARCHAR(20) UNIQUE,
  password_hash VARCHAR(255) NOT NULL,
  role VARCHAR(20) DEFAULT 'student',
  status VARCHAR(20) DEFAULT 'active',
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);

-- 用户资料表
CREATE TABLE user_profiles (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  real_name VARCHAR(50),
  avatar_url VARCHAR(255),
  grade INTEGER,
  class_name VARCHAR(50),
  school VARCHAR(100),
  location VARCHAR(100),
  bio TEXT,
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);
```

---

### 2.2 课程学习模块

#### 功能列表

- **课程体系**
  - 6大学科：数学、语文、英语、物理、化学、生物
  - 章节管理（每学科4-6个章节）
  - 课时管理（视频、文档、习题）

- **学习功能**
  - 视频播放（防盗链、倍速、断点续播）
  - 文档阅读
  - 学习进度追踪
  - 课程解锁机制（顺序学习）

- **学习记录**
  - 观看时长统计
  - 课程完成状态
  - 学习时间分布

#### 数据表设计

```sql
-- 学科表
CREATE TABLE subjects (
  id SERIAL PRIMARY KEY,
  name VARCHAR(50) NOT NULL,
  icon VARCHAR(10),
  color VARCHAR(20),
  description TEXT,
  sort_order INTEGER,
  status VARCHAR(20) DEFAULT 'active',
  created_at TIMESTAMP DEFAULT NOW()
);

-- 章节表
CREATE TABLE chapters (
  id SERIAL PRIMARY KEY,
  subject_id INTEGER REFERENCES subjects(id),
  title VARCHAR(100) NOT NULL,
  description TEXT,
  sort_order INTEGER,
  created_at TIMESTAMP DEFAULT NOW()
);

-- 课时表
CREATE TABLE lessons (
  id SERIAL PRIMARY KEY,
  chapter_id INTEGER REFERENCES chapters(id),
  title VARCHAR(100) NOT NULL,
  type VARCHAR(20), -- video, document, exercise
  content_url VARCHAR(255),
  duration INTEGER, -- 视频时长（秒）
  sort_order INTEGER,
  is_free BOOLEAN DEFAULT false,
  created_at TIMESTAMP DEFAULT NOW()
);

-- 学习进度表
CREATE TABLE user_learning_progress (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  lesson_id INTEGER REFERENCES lessons(id),
  progress INTEGER DEFAULT 0, -- 进度百分比
  duration INTEGER DEFAULT 0, -- 已学习时长
  completed BOOLEAN DEFAULT false,
  last_position INTEGER DEFAULT 0, -- 视频播放位置
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(user_id, lesson_id)
);
```

---

### 2.3 题库练习模块

#### 功能列表

- **题库管理**
  - 按学科分类
  - 按章节分类
  - 按难度分类（简单、中等、困难）
  - 题型支持（单选、多选、判断、填空、简答）

- **练习功能**
  - 练习集管理
  - 智能组卷
  - 答题计时
  - 自动批改（客观题）
  - 答题报告

- **错题本**
  - 错题记录
  - 错题分析
  - 错题重做

#### 数据表设计

```sql
-- 题目表
CREATE TABLE questions (
  id SERIAL PRIMARY KEY,
  subject_id INTEGER REFERENCES subjects(id),
  chapter_id INTEGER REFERENCES chapters(id),
  type VARCHAR(20), -- single_choice, multiple_choice, true_false, fill_blank, essay
  difficulty VARCHAR(20), -- easy, medium, hard
  content TEXT NOT NULL,
  answer TEXT, -- 标准答案
  explanation TEXT, -- 解析
  points INTEGER DEFAULT 1, -- 分值
  created_at TIMESTAMP DEFAULT NOW()
);

-- 选项表（用于选择题）
CREATE TABLE question_options (
  id SERIAL PRIMARY KEY,
  question_id INTEGER REFERENCES questions(id),
  option_key VARCHAR(10), -- A, B, C, D
  option_content TEXT NOT NULL,
  is_correct BOOLEAN DEFAULT false,
  sort_order INTEGER
);

-- 练习集表
CREATE TABLE question_sets (
  id SERIAL PRIMARY KEY,
  subject_id INTEGER REFERENCES subjects(id),
  title VARCHAR(100) NOT NULL,
  description TEXT,
  difficulty VARCHAR(20),
  question_count INTEGER,
  time_limit INTEGER, -- 时长限制（分钟）
  created_at TIMESTAMP DEFAULT NOW()
);

-- 答题记录表
CREATE TABLE user_answers (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  question_id INTEGER REFERENCES questions(id),
  question_set_id INTEGER REFERENCES question_sets(id),
  user_answer TEXT,
  is_correct BOOLEAN,
  score INTEGER,
  time_spent INTEGER, -- 答题用时（秒）
  created_at TIMESTAMP DEFAULT NOW()
);

-- 错题本表
CREATE TABLE wrong_questions (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  question_id INTEGER REFERENCES questions(id),
  wrong_count INTEGER DEFAULT 1,
  mastered BOOLEAN DEFAULT false,
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(user_id, question_id)
);
```

---

### 2.4 讨论社区模块

#### 功能列表

- **帖子管理**
  - 发布帖子
  - 编辑帖子
  - 删除帖子
  - 帖子分类（按学科）
  - 热门帖子

- **互动功能**
  - 评论回复
  - 点赞
  - 收藏
  - 搜索

- **内容审核**
  - 敏感词过滤
  - 人工审核

#### 数据表设计

```sql
-- 帖子表
CREATE TABLE posts (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  subject_id INTEGER REFERENCES subjects(id),
  title VARCHAR(200) NOT NULL,
  content TEXT NOT NULL,
  view_count INTEGER DEFAULT 0,
  like_count INTEGER DEFAULT 0,
  comment_count INTEGER DEFAULT 0,
  is_hot BOOLEAN DEFAULT false,
  status VARCHAR(20) DEFAULT 'published', -- draft, published, deleted
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);

-- 评论表
CREATE TABLE comments (
  id SERIAL PRIMARY KEY,
  post_id INTEGER REFERENCES posts(id),
  user_id INTEGER REFERENCES users(id),
  parent_id INTEGER REFERENCES comments(id), -- 回复的评论ID
  content TEXT NOT NULL,
  like_count INTEGER DEFAULT 0,
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);

-- 点赞表
CREATE TABLE post_likes (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  post_id INTEGER REFERENCES posts(id),
  created_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(user_id, post_id)
);

-- 收藏表
CREATE TABLE post_favorites (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  post_id INTEGER REFERENCES posts(id),
  created_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(user_id, post_id)
);
```

---

### 2.5 学习统计模块

#### 功能列表

- **首页概览**
  - 今日学习时长
  - 本周完成题目数
  - 累计获得勋章
  - 进步排名

- **学习数据**
  - 每周学习时长图表
  - 成绩趋势图表
  - 各科章节掌握度
  - 百分位排名
  - 练习平均分
  - 模拟考试平均分

- **今日任务**
  - 任务列表
  - 完成进度
  - 任务提醒

#### 数据表设计

```sql
-- 每日统计表
CREATE TABLE daily_stats (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  date DATE NOT NULL,
  study_duration INTEGER DEFAULT 0, -- 学习时长（分钟）
  questions_completed INTEGER DEFAULT 0, -- 完成题目数
  lessons_completed INTEGER DEFAULT 0, -- 完成课程数
  points_earned INTEGER DEFAULT 0, -- 获得积分
  created_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(user_id, date)
);

-- 学科统计表
CREATE TABLE subject_stats (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  subject_id INTEGER REFERENCES subjects(id),
  total_lessons INTEGER DEFAULT 0,
  completed_lessons INTEGER DEFAULT 0,
  total_questions INTEGER DEFAULT 0,
  correct_questions INTEGER DEFAULT 0,
  avg_score DECIMAL(5,2) DEFAULT 0,
  updated_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(user_id, subject_id)
);

-- 章节掌握度表
CREATE TABLE chapter_mastery (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  chapter_id INTEGER REFERENCES chapters(id),
  mastery_level INTEGER DEFAULT 0, -- 0-100
  practice_count INTEGER DEFAULT 0,
  correct_rate DECIMAL(5,2) DEFAULT 0,
  updated_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(user_id, chapter_id)
);
```

---

### 2.6 成长系统模块

#### 功能列表

- **等级系统**
  - 经验值累积
  - 等级提升
  - 等级特权

- **成就系统**
  - 成就列表
  - 成就解锁
  - 成就奖励

- **排行榜**
  - 班级排名
  - 年级排名
  - 学科排名
  - 周/月榜单

- **学习打卡**
  - 连续学习天数
  - 打卡奖励

#### 数据表设计

```sql
-- 用户等级表
CREATE TABLE user_levels (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id) UNIQUE,
  level INTEGER DEFAULT 1,
  experience INTEGER DEFAULT 0,
  next_level_exp INTEGER DEFAULT 100,
  updated_at TIMESTAMP DEFAULT NOW()
);

-- 成就表
CREATE TABLE achievements (
  id SERIAL PRIMARY KEY,
  name VARCHAR(100) NOT NULL,
  description TEXT,
  icon VARCHAR(50),
  type VARCHAR(50), -- beginner, diligent, master, perfect_score, etc.
  condition_type VARCHAR(50), -- lessons_completed, questions_correct, streak_days
  condition_value INTEGER,
  reward_exp INTEGER DEFAULT 0,
  created_at TIMESTAMP DEFAULT NOW()
);

-- 用户成就表
CREATE TABLE user_achievements (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  achievement_id INTEGER REFERENCES achievements(id),
  progress INTEGER DEFAULT 0,
  unlocked BOOLEAN DEFAULT false,
  unlocked_at TIMESTAMP,
  UNIQUE(user_id, achievement_id)
);

-- 排行榜表（Redis 实时计算，数据库定期快照）
CREATE TABLE rankings (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  rank_type VARCHAR(50), -- class, grade, subject
  rank_value INTEGER, -- 排名
  score INTEGER, -- 积分
  period VARCHAR(50), -- week, month
  snapshot_date DATE,
  created_at TIMESTAMP DEFAULT NOW()
);

-- 学习打卡表
CREATE TABLE user_checkins (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  checkin_date DATE NOT NULL,
  streak_days INTEGER DEFAULT 1,
  created_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(user_id, checkin_date)
);
```

---

## 3. 技术架构设计

### 3.1 整体架构图

```
┌─────────────────────────────────────────────────┐
│                   客户端层                       │
│  React 18 + TypeScript + shadcn/ui              │
│  (响应式设计，支持桌面端和移动端)                 │
└─────────────────────────────────────────────────┘
                      ↓ HTTPS
┌─────────────────────────────────────────────────┐
│                   CDN 层                         │
│  静态资源、视频、图片加速                         │
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
│                  网关层 / 负载均衡                │
│  Nginx / API Gateway                            │
│  功能：路由、认证、限流、熔断                      │
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
│                  应用层（业务逻辑）                │
│  ┌──────────┬──────────┬──────────┬──────────┐ │
│  │ 用户服务 │ 课程服务 │ 题库服务 │ 统计服务 │ │
│  │  User    │ Course   │ Question │   Stat   │ │
│  │ Service  │ Service  │ Service  │ Service  │ │
│  └──────────┴──────────┴──────────┴──────────┘ │
│  ┌──────────┬──────────┬──────────┬──────────┐ │
│  │ 讨论服务 │ 成长服务 │ 消息服务 │ 搜索服务 │ │
│  │Community │ Growth   │Notification│ Search  │ │
│  │ Service  │ Service  │ Service  │ Service  │ │
│  └──────────┴──────────┴──────────┴──────────┘ │
│                                                  │
│  技术栈：NestJS + Prisma                         │
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
│              数据层（持久化存储）                  │
│  ┌──────────┬──────────┬──────────┬──────────┐ │
│  │PostgreSQL│  Redis   │ MongoDB  │   OSS    │ │
│  │ 关系数据 │   缓存   │  文档型  │ 对象存储  │ │
│  └──────────┴──────────┴──────────┴──────────┘ │
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
│              基础设施层（中间件）                  │
│  ┌──────────┬──────────┬──────────┬──────────┐ │
│  │消息队列  │ 搜索引擎 │ 监控告警 │ 日志系统 │ │
│  │RabbitMQ  │Elasticsearch│Prometheus│   ELK   │ │
│  └──────────┴──────────┴──────────┴──────────┘ │
└─────────────────────────────────────────────────┘
```

---

### 3.2 技术栈选型

#### 前端技术栈

```yaml
框架: React 18
语言: TypeScript
构建工具: Vite
UI 组件库: shadcn/ui (基于 Radix UI)
状态管理: Zustand / Redux Toolkit
路由: React Router v6
HTTP 客户端: Axios + React Query
表单验证: Zod + React Hook Form
图表: Recharts
富文本编辑器: Tiptap
CSS: Tailwind CSS
国际化: i18next
主题: CSS 变量（支持亮色/暗色模式）
```

#### 后端技术栈

```yaml
框架: NestJS
语言: TypeScript
ORM: Prisma
数据库: PostgreSQL 14+
缓存: Redis 7+
对象存储: 阿里云 OSS / AWS S3
消息队列: RabbitMQ
搜索引擎: Elasticsearch
API 文档: Swagger / OpenAPI
认证: JWT + Passport
验证: class-validator
日志: Winston
监控: Prometheus + Grafana
```

#### 部署运维

```yaml
容器化: Docker
编排: Kubernetes (可选)
CI/CD: GitHub Actions / GitLab CI
前端托管: Vercel / Netlify / Cloudflare Pages
后端托管: AWS / 阿里云 / 腾讯云
数据库: RDS (云数据库)
域名: 自定义域名
SSL证书: Let's Encrypt (免费) / 云厂商 SSL
监控: Sentry (错误监控) + Google Analytics
```

---

### 3.3 为什么选择 NestJS？

#### 优势

1. ✅ **TypeScript 原生支持**：前后端统一语言，类型共享
2. ✅ **企业级框架**：模块化设计，易于维护和扩展
3. ✅ **依赖注入**：代码解耦，方便测试
4. ✅ **丰富的生态**：官方提供大量模块（认证、缓存、队列等）
5. ✅ **性能优异**：基于 Express/Fastify，高性能
6. ✅ **与前端技术栈统一**：降低学习成本

#### 与其他框架对比

| 特性       | NestJS     | Spring Boot | Django     | FastAPI    |
| ---------- | ---------- | ----------- | ---------- | ---------- |
| 语言       | TypeScript | Java        | Python     | Python     |
| 学习曲线   | 中等       | 陡峭        | 平缓       | 平缓       |
| 性能       | ⭐⭐⭐⭐   | ⭐⭐⭐⭐⭐  | ⭐⭐⭐     | ⭐⭐⭐⭐   |
| 生态       | ⭐⭐⭐⭐   | ⭐⭐⭐⭐⭐  | ⭐⭐⭐⭐⭐ | ⭐⭐⭐     |
| 开发效率   | ⭐⭐⭐⭐⭐ | ⭐⭐⭐      | ⭐⭐⭐⭐   | ⭐⭐⭐⭐⭐ |
| 企业级特性 | ⭐⭐⭐⭐   | ⭐⭐⭐⭐⭐  | ⭐⭐⭐⭐   | ⭐⭐⭐     |
| 前后端统一 | ✅         | ❌          | ❌         | ❌         |

---

### 3.4 微服务拆分建议（V2.0）

#### MVP/V1.0 阶段：单体架构

- 所有模块在一个 NestJS 应用中
- 通过模块（Module）进行逻辑隔离
- 部署简单，开发效率高

#### V2.0 阶段：微服务架构

当用户量超过 10000 时，可以考虑拆分为微服务：

```yaml
用户服务 (User Service):
  - 用户注册登录
  - 用户信息管理
  - 权限管理

课程服务 (Course Service):
  - 课程管理
  - 学习进度
  - 视频管理

题库服务 (Question Service):
  - 题目管理
  - 答题记录
  - 错题本

统计服务 (Stats Service):
  - 学习统计
  - 数据分析
  - 报表生成

社区服务 (Community Service):
  - 帖子管理
  - 评论管理
  - 搜索功能

成长服务 (Growth Service):
  - 等级系统
  - 成就系统
  - 排行榜
```

---

## 4. 数据库设计

### 4.1 数据库 ER 图

```
┌────────────┐       ┌────────────┐       ┌────────────┐
│   users    │──1:1──│user_profiles│       │  subjects  │
│    用户表   │       │  用户资料表  │       │   学科表    │
└────────────┘       └────────────┘       └────────────┘
      │                                          │
      │1                                         │1
      │                                          │
      │N                                         │N
┌────────────┐       ┌────────────┐       ┌────────────┐
│user_learning│       │  chapters  │───1:N──│  lessons   │
│_progress   │       │   章节表    │       │   课时表    │
│学习进度表   │       └────────────┘       └────────────┘
└────────────┘
      │
      │N
      │
      │1
┌────────────┐       ┌────────────┐       ┌────────────┐
│  questions │───1:N──│question_   │       │question_   │
│   题目表    │       │options     │       │sets        │
└────────────┘       │  选项表     │       │ 练习集表    │
      │              └────────────┘       └────────────┘
      │                                          │
      │                                          │
      │                  ┌────────────┐          │
      └──────────────────│user_answers│──────────┘
                         │ 答题记录表  │
                         └────────────┘
```

### 4.2 核心表结构汇总

#### 用户相关（3张表）

- `users` - 用户基本信息
- `user_profiles` - 用户详细资料
- `user_achievements` - 用户成就

#### 课程相关（4张表）

- `subjects` - 学科
- `chapters` - 章节
- `lessons` - 课时
- `user_learning_progress` - 学习进度

#### 题库相关（5张表）

- `questions` - 题目
- `question_options` - 选项
- `question_sets` - 练习集
- `user_answers` - 答题记录
- `wrong_questions` - 错题本

#### 社区相关（4张表）

- `posts` - 帖子
- `comments` - 评论
- `post_likes` - 点赞
- `post_favorites` - 收藏

#### 统计相关（4张表）

- `daily_stats` - 每日统计
- `subject_stats` - 学科统计
- `chapter_mastery` - 章节掌握度
- `rankings` - 排行榜

#### 成长相关（4张表）

- `user_levels` - 用户等级
- `achievements` - 成就定义
- `user_achievements` - 用户成就记录
- `user_checkins` - 学习打卡

**总计：约 25 张核心表**

---

### 4.3 索引设计

#### 高频查询索引

```sql
-- 用户表
CREATE INDEX idx_users_email ON users(email);
CREATE INDEX idx_users_phone ON users(phone);
CREATE INDEX idx_users_role ON users(role);

-- 学习进度表
CREATE INDEX idx_progress_user_lesson ON user_learning_progress(user_id, lesson_id);
CREATE INDEX idx_progress_user ON user_learning_progress(user_id);

-- 答题记录表
CREATE INDEX idx_answers_user_question ON user_answers(user_id, question_id);
CREATE INDEX idx_answers_user ON user_answers(user_id);
CREATE INDEX idx_answers_created ON user_answers(created_at);

-- 帖子表
CREATE INDEX idx_posts_user ON posts(user_id);
CREATE INDEX idx_posts_subject ON posts(subject_id);
CREATE INDEX idx_posts_hot ON posts(is_hot);
CREATE INDEX idx_posts_created ON posts(created_at);

-- 评论表
CREATE INDEX idx_comments_post ON comments(post_id);
CREATE INDEX idx_comments_user ON comments(user_id);

-- 每日统计表
CREATE INDEX idx_daily_stats_user_date ON daily_stats(user_id, date);
```

---

### 4.4 分库分表策略（大规模场景）

当数据量超过 1000 万条时，需要考虑分库分表：

#### 分表策略

```yaml
答题记录表 (user_answers):
  分表方式: 按用户 ID 哈希分表
  分表数量: 16 或 32 张
  命名规则: user_answers_0, user_answers_1, ...

  示例：
  user_id = 12345
  table_index = 12345 % 16 = 9
  表名 = user_answers_9

学习进度表 (user_learning_progress):
  分表方式: 按用户 ID 哈希分表
  分表数量: 16 张

每日统计表 (daily_stats):
  分表方式: 按时间范围分表（按月）
  命名规则: daily_stats_202401, daily_stats_202402, ...
```

#### 分库策略（超大规模）

```yaml
用户量超过 100 万时:
  - 用户库（用户相关表）
  - 课程库（课程相关表）
  - 题库（题目相关表）
  - 统计库（统计相关表）
```

---

## 5. API 接口设计

### 5.1 RESTful API 设计规范

#### 基础规则

```
- 使用名词复数：/api/users, /api/courses
- HTTP 方法语义化：
  - GET: 获取资源
  - POST: 创建资源
  - PUT: 更新资源（全量）
  - PATCH: 更新资源（部分）
  - DELETE: 删除资源
- 版本控制：/api/v1/users
- 统一响应格式
```

#### 统一响应格式

```typescript
// 成功响应
{
  "success": true,
  "data": {
    // 数据内容
  },
  "message": "操作成功",
  "timestamp": 1640000000000
}

// 失败响应
{
  "success": false,
  "error": {
    "code": "USER_NOT_FOUND",
    "message": "用户不存在"
  },
  "timestamp": 1640000000000
}

// 分页响应
{
  "success": true,
  "data": {
    "items": [],
    "pagination": {
      "page": 1,
      "pageSize": 20,
      "total": 100,
      "totalPages": 5
    }
  }
}
```

---

### 5.2 核心 API 接口列表

#### 用户模块

```yaml
POST   /api/auth/register           # 用户注册
POST   /api/auth/login              # 用户登录
POST   /api/auth/logout             # 用户登出
POST   /api/auth/refresh            # 刷新 token
POST   /api/auth/send-code          # 发送验证码
POST   /api/auth/reset-password     # 重置密码

GET    /api/users/me                # 获取当前用户信息
PUT    /api/users/me                # 更新用户信息
PUT    /api/users/password          # 修改密码
POST   /api/users/avatar            # 上传头像
```

#### 课程模块

```yaml
GET    /api/subjects                # 获取所有学科
GET    /api/subjects/:id            # 获取学科详情
GET    /api/subjects/:id/chapters   # 获取学科章节列表
GET    /api/chapters/:id            # 获取章节详情
GET    /api/chapters/:id/lessons    # 获取章节课时列表
GET    /api/lessons/:id             # 获取课时详情
GET    /api/lessons/:id/video-url   # 获取视频播放地址

POST   /api/learning/progress       # 更新学习进度
GET    /api/learning/progress       # 获取学习进度
GET    /api/learning/recent         # 获取最近学习记录
```

#### 题库模块

```yaml
GET    /api/questions               # 获取题目列表（支持筛选）
  查询参数:
    - subject_id: 学科ID
    - chapter_id: 章节ID
    - difficulty: 难度
    - type: 题型
    - page: 页码
    - page_size: 每页数量

GET    /api/questions/:id           # 获取题目详情
GET    /api/question-sets           # 获取练习集列表
GET    /api/question-sets/:id       # 获取练习集详情
POST   /api/answers                 # 提交答案
GET    /api/answers/stats           # 获取答题统计
GET    /api/answers/report/:id      # 获取答题报告
GET    /api/wrong-questions         # 获取错题本
POST   /api/wrong-questions/:id/master  # 标记题目已掌握
```

#### 讨论模块

```yaml
GET    /api/posts                   # 获取帖子列表
  查询参数:
    - subject_id: 学科ID
    - sort: 排序方式（hot, new）
    - keyword: 搜索关键词
    - page: 页码

POST   /api/posts                   # 发布帖子
GET    /api/posts/:id               # 获取帖子详情
PUT    /api/posts/:id               # 编辑帖子
DELETE /api/posts/:id               # 删除帖子

GET    /api/posts/:id/comments      # 获取帖子评论
POST   /api/posts/:id/comments      # 发表评论
POST   /api/posts/:id/like          # 点赞帖子
DELETE /api/posts/:id/like          # 取消点赞
POST   /api/posts/:id/favorite      # 收藏帖子
DELETE /api/posts/:id/favorite      # 取消收藏

GET    /api/search/posts            # 搜索帖子
```

#### 统计模块

```yaml
GET    /api/stats/overview          # 首页概览统计
  返回数据:
    - today_study_time: 今日学习时长
    - week_questions_count: 本周完成题目数
    - total_badges: 累计勋章数
    - progress_rank: 进步排名

GET    /api/stats/today-tasks       # 获取今日任务
GET    /api/stats/performance       # 获取学习成绩统计
GET    /api/stats/weekly-chart      # 获取每周学习时长图表
GET    /api/stats/score-trend       # 获取成绩趋势图表
GET    /api/stats/subject/:id/chapters  # 获取学科章节掌握度
```

#### 成长模块

```yaml
GET    /api/growth/level            # 获取用户等级信息
GET    /api/growth/achievements     # 获取成就列表
POST   /api/growth/checkin          # 学习打卡
GET    /api/growth/streak           # 获取连续学习天数

GET    /api/rankings                # 获取排行榜
  查询参数:
    - type: 排名类型（class, grade, subject）
    - subject_id: 学科ID（可选）
    - period: 时间范围（week, month）
    - limit: 返回数量
```

#### 通知模块

```yaml
GET    /api/notifications           # 获取通知列表
PUT    /api/notifications/:id/read  # 标记通知已读
PUT    /api/notifications/read-all  # 标记全部已读
```

---

### 5.3 API 认证与鉴权

#### JWT Token 方案

```typescript
// Access Token (短期，1小时)
{
  "userId": 12345,
  "role": "student",
  "exp": 1640000000
}

// Refresh Token (长期，7天)
{
  "userId": 12345,
  "tokenId": "uuid",
  "exp": 1640604800
}
```

#### 请求头格式

```
Authorization: Bearer <access_token>
```

#### Token 刷新流程

```
1. Access Token 过期
2. 前端使用 Refresh Token 请求 /api/auth/refresh
3. 后端验证 Refresh Token，返回新的 Access Token
4. 前端更新 Token，重试原请求
```

---

### 5.4 API 限流策略

```yaml
普通接口:
  - 限制: 100 次/分钟/IP
  - 超出后: 返回 429 Too Many Requests

登录接口:
  - 限制: 5 次/分钟/IP
  - 超出后: 需要验证码

发送验证码:
  - 限制: 1 次/分钟/手机号
  - 限制: 10 次/小时/手机号

答题提交:
  - 限制: 200 次/分钟/用户
  - 防止作弊

搜索接口:
  - 限制: 30 次/分钟/用户
  - 防止爬虫
```

---

## 6. 开发路线图

### 6.1 项目准备阶段（Week 0-1）

#### Week 0: 团队组建与环境准备

- [ ] 团队成员到位（产品、前端×2、后端×2、UI、测试）
- [ ] 开发环境搭建（电脑、软件、账号）
- [ ] 协作工具配置（Git、项目管理工具）
- [ ] 技术选型确认
- [ ] 制定开发规范（代码规范、Git 规范、命名规范）

#### Week 1: 技术架构搭建

- [ ] 创建 Git 仓库（Monorepo / 多仓库）
- [ ] 前端项目初始化（React + Vite + TypeScript）
- [ ] 后端项目初始化（NestJS + Prisma）
- [ ] 配置 ESLint、Prettier、Husky
- [ ] 配置 CI/CD 流水线
- [ ] 数据库设计（ER图、表结构）
- [ ] 购买域名、云服务器（可选）

---

### 6.2 MVP 阶段（Week 2-8，约2个月）

#### Week 2-3: 用户系统 + 基础架构

**后端开发任务**

- [ ] 项目结构搭建（模块化设计）
- [ ] 数据库连接配置（PostgreSQL + Prisma）
- [ ] Redis 连接配置
- [ ] 用户注册 API
  - [ ] 手机号/邮箱验证
  - [ ] 密码加密（bcrypt）
  - [ ] 验证码发送（阿里云短信）
- [ ] 用户登录 API
  - [ ] JWT token 生成
  - [ ] Refresh token 机制
  - [ ] Redis 存储 session
- [ ] 用户信息管理 API
  - [ ] 获取/更新用户信息
  - [ ] 头像上传（OSS）
- [ ] 权限系统（RBAC）
- [ ] 统一异常处理
- [ ] 统一响应格式
- [ ] API 日志记录
- [ ] Swagger API 文档

**前端开发任务**

- [ ] 路由配置（React Router）
- [ ] 状态管理配置（Zustand）
- [ ] API 请求封装（Axios + React Query）
- [ ] 登录页面
  - [ ] 手机号/邮箱登录
  - [ ] 验证码登录
  - [ ] 表单验证（Zod）
- [ ] 注册页面
- [ ] 用户信息页面
- [ ] 全局状态管理
  - [ ] 用户状态
  - [ ] Token 管理
  - [ ] 主题切换
  - [ ] 语言切换
- [ ] 路由守卫（登录验证）
- [ ] 404 页面

**测试与部署**

- [ ] 单元测试（Jest）
- [ ] 部署到测试环境
- [ ] 内部测试

**里程碑：Week 3 完成可注册登录的 Demo**

---

#### Week 4-5: 课程学习模块

**后端开发任务**

- [ ] 课程数据表设计与创建
  - [ ] subjects（学科表）
  - [ ] chapters（章节表）
  - [ ] lessons（课时表）
  - [ ] user_learning_progress（学习进度表）
- [ ] 课程管理 API
  - [ ] 获取学科列表
  - [ ] 获取章节列表
  - [ ] 获取课时列表
  - [ ] 获取课时详情
- [ ] 学习进度 API
  - [ ] 记录学习进度
  - [ ] 获取学习进度
  - [ ] 课程解锁逻辑
- [ ] 视频服务
  - [ ] OSS 配置
  - [ ] 视频上传接口
  - [ ] 防盗链签名 URL 生成

**前端开发任务**

- [ ] 学习页面开发
  - [ ] 科目选择标签（Tabs）
  - [ ] 章节列表（Accordion）
  - [ ] 课时列表
  - [ ] 进度条显示
  - [ ] 锁定/解锁状态
- [ ] 视频播放器集成
  - [ ] Video.js / Plyr
  - [ ] 播放进度记录
  - [ ] 倍速播放
  - [ ] 全屏功能
  - [ ] 断点续播
- [ ] 学习统计组件

**数据准备**

- [ ] 准备 6 个学科的课程数据
- [ ] 每个学科 3-4 个章节
- [ ] 每个章节 4-5 个课时
- [ ] 准备测试视频

**里程碑：Week 5 完成可观看视频、记录进度的版本**

---

#### Week 6-7: 题库练习模块

**后端开发任务**

- [ ] 题库数据表设计
  - [ ] questions（题目表）
  - [ ] question_options（选项表）
  - [ ] question_sets（练习集表）
  - [ ] user_answers（答题记录表）
  - [ ] wrong_questions（错题本表）
- [ ] 题库管理 API
  - [ ] 获取题目列表（支持筛选）
  - [ ] 获取练习集
  - [ ] 提交答案（自动批改）
  - [ ] 获取答题统计
  - [ ] 错题本管理
- [ ] 答题报告生成逻辑

**前端开发任务**

- [ ] 题库页面开发
  - [ ] 科目统计卡片
  - [ ] 练习集筛选器
  - [ ] 练习集列表
  - [ ] 完成状态标记
- [ ] 答题页面
  - [ ] 题目展示
  - [ ] 单选/多选/判断题组件
  - [ ] 倒计时功能
  - [ ] 答题卡
  - [ ] 交卷确认
- [ ] 答题报告页面
- [ ] 错题本页面

**数据准备**

- [ ] 准备 500+ 题目
- [ ] 每个学科 80-100 题
- [ ] 配置标准答案和解析

**里程碑：Week 7 完成可做题、查看报告的版本**

---

#### Week 8: 首页数据统计

**后端开发任务**

- [ ] 统计数据表设计
  - [ ] daily_stats（每日统计）
  - [ ] subject_stats（学科统计）
  - [ ] chapter_mastery（章节掌握度）
- [ ] 统计数据 API
  - [ ] 首页概览数据
  - [ ] 今日任务列表
  - [ ] 学习成绩统计
  - [ ] 科目章节掌握度
- [ ] 定时任务（每天凌晨统计）
- [ ] Redis 缓存优化

**前端开发任务**

- [ ] 首页开发
  - [ ] 欢迎横幅
  - [ ] 统计卡片（4个）
  - [ ] 今日任务进度条
  - [ ] 圆形进度图（Recharts）
  - [ ] 科目章节掌握度

**里程碑：Week 8 完成 MVP 版本，内部测试**

---

### 6.3 V1.0 阶段（Week 9-16，约2个月）

#### Week 9-10: 讨论区模块

**后端开发任务**

- [ ] 讨论区数据表设计
  - [ ] posts（帖子表）
  - [ ] comments（评论表）
  - [ ] post_likes（点赞表）
- [ ] 讨论区 API
  - [ ] 发布帖子
  - [ ] 获取帖子列表（分页、排序）
  - [ ] 获取帖子详情
  - [ ] 发表评论
  - [ ] 点赞/取消点赞
  - [ ] 搜索帖子
- [ ] 消息通知系统
  - [ ] 评论通知
  - [ ] 点赞通知

**前端开发任务**

- [ ] 讨论区页面
  - [ ] 统计卡片
  - [ ] 搜索框
  - [ ] 筛选器
  - [ ] 帖子列表
  - [ ] 发布帖子弹窗
- [ ] 帖子详情页
- [ ] 评论组件
- [ ] 富文本编辑器（Tiptap）

---

#### Week 11-12: 成长系统模块

**后端开发任务**

- [ ] 成长系统数据表设计
  - [ ] user_levels（等级表）
  - [ ] achievements（成就表）
  - [ ] user_achievements（用户成就表）
  - [ ] rankings（排行榜表）
  - [ ] user_checkins（打卡表）
- [ ] 成长系统 API
  - [ ] 获取用户等级
  - [ ] 获取成就列表
  - [ ] 解锁成就
  - [ ] 学习打卡
  - [ ] 获取排行榜
- [ ] 排行榜实现（Redis Sorted Set）

**前端开发任务**

- [ ] 成长页面
  - [ ] 成长概览卡片
  - [ ] 学习数据图表
  - [ ] 科目进度展示
  - [ ] 成就系统
  - [ ] 排行榜

---

#### Week 13: 用户中心完善

**后端开发任务**

- [ ] 用户设置 API
  - [ ] 修改密码
  - [ ] 绑定邮箱/手机
  - [ ] 通知设置
  - [ ] 学习偏好设置

**前端开发任务**

- [ ] 用户中心页面
  - [ ] 基本资料 Tab
  - [ ] 学习设置 Tab
  - [ ] 通知设置 Tab
  - [ ] 安全设置 Tab

---

#### Week 14-15: 性能优化与完善

**后端优化**

- [ ] 数据库索引优化
- [ ] SQL 查询优化
- [ ] Redis 缓存策略
- [ ] API 响应时间优化（<200ms）
- [ ] 安全加固
  - [ ] SQL 注入防护
  - [ ] XSS 防护
  - [ ] CSRF 防护
  - [ ] API 限流

**前端优化**

- [ ] 代码分割（React.lazy）
- [ ] 图片懒加载
- [ ] Bundle 大小优化
- [ ] Loading 状态优化
- [ ] Skeleton 加载效果
- [ ] 错误边界
- [ ] 响应式适配测试

**测试**

- [ ] 单元测试（覆盖率 >80%）
- [ ] 集成测试
- [ ] E2E 测试（Playwright）
- [ ] 性能测试（JMeter）
- [ ] 安全测试

---

#### Week 16: 部署上线

**部署准备**

- [ ] 购买域名
- [ ] 配置 SSL 证书
- [ ] 配置 CDN
- [ ] 购买云服务器

**生产环境部署**

- [ ] Docker 容器化
- [ ] 前端部署（Vercel）
- [ ] 后端部署（阿里云）
- [ ] 数据库部署（RDS）
- [ ] Redis 部署

**监控与告警**

- [ ] Sentry 错误监控
- [ ] Google Analytics
- [ ] 日志收集（ELK）
- [ ] 性能监控（Prometheus）

**里程碑：Week 16 正式上线 V1.0**

---

### 6.4 V2.0 阶段（后续迭代）

#### 功能迭代清单

1. **AI 智能推荐系统**
   - 个性化学习路径
   - 智能题目推荐
   - 薄弱知识点识别

2. **智能批改系统**
   - 主观题 AI 批改
   - 作文批改与评分

3. **直播课程**
   - 直播系统搭建
   - 直播间互动
   - 直播回放

4. **在线答疑**
   - 实时聊天（WebSocket）
   - 一对一辅导
   - 语音/视频通话

5. **家长端**
   - 家长账号系统
   - 学习报告推送
   - 成绩分析

6. **社区化功能**
   - 学习小组
   - 同学关注
   - 学习动态

7. **游戏化元素**
   - 积分系统
   - 虚拟货币
   - PK 竞赛模式

8. **移动端 App**
   - React Native 开发
   - iOS/Android 版本

---

## 7. 技术难点与解决方案

### 7.1 学习进度实时同步

#### 难点分析

- 用户观看视频时需要实时记录进度
- 多端同步（手机、平板、电脑）
- 断点续播
- 高频写入可能影响数据库性能

#### 解决方案

**方案一：定时上报 + Redis 缓存**

```typescript
// 前端：每 30 秒上报一次进度
setInterval(() => {
  updateProgress({
    lessonId,
    position: videoPlayer.currentTime,
    duration: videoPlayer.duration
  });
}, 30000);

// 后端：先写 Redis，定期批量写入数据库
@Post('learning/progress')
async updateProgress(@Body() dto: UpdateProgressDto) {
  // 1. 更新 Redis 缓存（实时）
  await this.redis.set(
    `progress:${userId}:${lessonId}`,
    JSON.stringify(dto),
    'EX', 3600
  );

  // 2. 发送消息到队列（异步批量写入数据库）
  await this.queue.add('sync-progress', { userId, lessonId, ...dto });

  return { success: true };
}

// 消费者：批量写入数据库
@Process('sync-progress')
async syncProgress(job: Job) {
  const { userId, lessonId, position, duration } = job.data;

  await this.prisma.userLearningProgress.upsert({
    where: { userId_lessonId: { userId, lessonId } },
    update: { position, duration, updatedAt: new Date() },
    create: { userId, lessonId, position, duration }
  });
}
```

**方案二：WebSocket 实时同步（多端同步）**

```typescript
// 客户端 A 更新进度
socket.emit('progress:update', { lessonId, position })

// 服务端广播到客户端 B
io.to(`user:${userId}`).emit('progress:sync', { lessonId, position })

// 客户端 B 同步进度
socket.on('progress:sync', (data) => {
  videoPlayer.currentTime = data.position
})
```

---

### 7.2 答题批改与统计

#### 难点分析

- 实时批改大量试卷
- 复杂的统计分析（各维度掌握度）
- 历史数据量巨大（百万级答题记录）

#### 解决方案

**客观题自动批改**

```typescript
@Post('answers')
async submitAnswer(@Body() dto: SubmitAnswerDto) {
  const question = await this.getQuestion(dto.questionId);

  // 前端已经比对答案，后端只做验证和记录
  const isCorrect = this.verifyAnswer(question, dto.userAnswer);

  // 异步记录答题记录（不阻塞响应）
  this.queue.add('record-answer', {
    userId: dto.userId,
    questionId: dto.questionId,
    userAnswer: dto.userAnswer,
    isCorrect,
    timeSpent: dto.timeSpent
  });

  // 立即返回结果
  return {
    isCorrect,
    correctAnswer: question.answer,
    explanation: question.explanation
  };
}
```

**统计分析优化**

```typescript
// 1. 预计算：定时任务每天凌晨计算统计数据
@Cron('0 0 * * *') // 每天凌晨执行
async calculateDailyStats() {
  const users = await this.getActiveUsers();

  for (const user of users) {
    // 计算各维度统计
    const stats = await this.calculateUserStats(user.id);

    // 存储到统计表
    await this.saveStats(user.id, stats);
  }
}

// 2. 缓存：热点统计数据缓存到 Redis
@Get('stats/overview')
async getOverview(@User() user) {
  // 先查 Redis
  const cached = await this.redis.get(`stats:overview:${user.id}`);
  if (cached) return JSON.parse(cached);

  // 查数据库
  const stats = await this.getStatsFromDB(user.id);

  // 写入 Redis（1小时过期）
  await this.redis.set(
    `stats:overview:${user.id}`,
    JSON.stringify(stats),
    'EX', 3600
  );

  return stats;
}
```

**大数据量处理**

```typescript
// 使用 ClickHouse 存储海量答题日志
// PostgreSQL 只存储汇总数据，ClickHouse 存储明细数据

// 查询历史答题记录（ClickHouse）
SELECT
  subject_id,
  COUNT(*) as total,
  SUM(is_correct) as correct,
  AVG(time_spent) as avg_time
FROM answer_logs
WHERE user_id = 12345
  AND created_at >= '2024-01-01'
GROUP BY subject_id
```

---

### 7.3 排行榜系统

#### 难点分析

- 实时更新
- 高并发读取
- 多维度排名（全校、班级、学科）

#### 解决方案

**Redis Sorted Set 实现排行榜**

```typescript
// 更新用户积分
async updateRanking(userId: number, score: number) {
  // 全校排行榜
  await this.redis.zadd('ranking:school:week', score, userId);

  // 班级排行榜
  const classId = await this.getUserClassId(userId);
  await this.redis.zadd(`ranking:class:${classId}:week`, score, userId);

  // 学科排行榜
  await this.redis.zadd(`ranking:subject:math:week`, score, userId);
}

// 获取排行榜
async getRanking(type: string, limit: number = 100) {
  const key = `ranking:${type}:week`;

  // 获取前 N 名（分数从高到低）
  const rankings = await this.redis.zrevrange(
    key,
    0,
    limit - 1,
    'WITHSCORES'
  );

  return this.formatRankings(rankings);
}

// 获取用户排名
async getUserRank(userId: number) {
  const rank = await this.redis.zrevrank('ranking:school:week', userId);
  return rank + 1; // 排名从 1 开始
}

// 定时任务：每周一重置排行榜
@Cron('0 0 * * 1') // 每周一凌晨执行
async resetWeeklyRanking() {
  // 快照到数据库
  await this.snapshotRanking('ranking:school:week');

  // 删除 Redis 中的数据
  await this.redis.del('ranking:school:week');
}
```

---

### 7.4 视频播放与防盗链

#### 难点分析

- 视频资源防盗
- 播放流畅度
- 带宽成本控制

#### 解决方案

**防盗链：URL 签名**

```typescript
@Get('lessons/:id/video-url')
async getVideoUrl(@Param('id') lessonId: number, @User() user) {
  // 1. 验证用户权限
  const hasPermission = await this.checkPermission(user.id, lessonId);
  if (!hasPermission) throw new ForbiddenException();

  // 2. 生成签名 URL（有效期 1 小时）
  const videoKey = await this.getVideoKey(lessonId);
  const signedUrl = this.ossService.generateSignedUrl(videoKey, 3600);

  return { url: signedUrl };
}

// OSS 签名 URL 生成
generateSignedUrl(key: string, expiresIn: number) {
  const expires = Date.now() + expiresIn * 1000;
  const signature = crypto
    .createHmac('sha256', this.secretKey)
    .update(`${key}:${expires}`)
    .digest('hex');

  return `${this.ossEndpoint}/${key}?expires=${expires}&signature=${signature}`;
}
```

**流畅播放：CDN + HLS 切片**

```typescript
// 视频上传时自动转码为 HLS 格式
async uploadVideo(file: Express.Multer.File) {
  // 1. 上传到 OSS
  const objectKey = await this.ossService.upload(file);

  // 2. 触发转码任务（阿里云视频点播服务）
  const transcodeJob = await this.vodService.createTranscodeJob({
    input: objectKey,
    output: {
      format: 'HLS', // HTTP Live Streaming
      resolutions: ['720p', '480p', '360p'] // 多清晰度
    }
  });

  return { jobId: transcodeJob.id };
}

// 前端播放器自适应清晰度
<video>
  <source src="https://cdn.example.com/video/lesson123.m3u8" type="application/x-mpegURL" />
</video>
```

**成本优化**

```yaml
策略: 1. 热门课程预热到 CDN（减少回源）
  2. 冷门课程设置过期时间（节省存储）
  3. 低分辨率优先（节省带宽）
  4. 监控流量异常（防止盗链）
```

---

### 7.5 高并发场景处理

#### 难点分析

- 考试高峰期大量学生同时答题
- 排行榜刷新压力
- 讨论区热帖访问

#### 解决方案

**限流降级**

```typescript
// 使用 Redis + 令牌桶算法限流
@UseGuards(ThrottlerGuard)
@Throttle(100, 60) // 每分钟 100 次
@Post('answers')
async submitAnswer(@Body() dto: SubmitAnswerDto) {
  // ...
}

// 自定义限流器
@Injectable()
export class RateLimiter {
  async checkLimit(key: string, limit: number, window: number): Promise<boolean> {
    const current = await this.redis.incr(key);

    if (current === 1) {
      await this.redis.expire(key, window);
    }

    return current <= limit;
  }
}

// 熔断机制
@Injectable()
export class CircuitBreaker {
  private failures = 0;
  private state = 'CLOSED'; // CLOSED, OPEN, HALF_OPEN

  async call(fn: Function) {
    if (this.state === 'OPEN') {
      throw new ServiceUnavailableException('服务暂时不可用');
    }

    try {
      const result = await fn();
      this.onSuccess();
      return result;
    } catch (error) {
      this.onFailure();
      throw error;
    }
  }

  private onFailure() {
    this.failures++;
    if (this.failures >= 5) {
      this.state = 'OPEN';
      // 30 秒后尝试恢复
      setTimeout(() => { this.state = 'HALF_OPEN'; }, 30000);
    }
  }

  private onSuccess() {
    this.failures = 0;
    this.state = 'CLOSED';
  }
}
```

**数据库优化**

```sql
-- 读写分离配置（Prisma）
const prisma = new PrismaClient({
  datasources: {
    db: {
      url: process.env.DATABASE_WRITE_URL, -- 主库（写）
    },
  },
  replicas: [
    { url: process.env.DATABASE_READ_URL_1 }, -- 从库 1（读）
    { url: process.env.DATABASE_READ_URL_2 }, -- 从库 2（读）
  ],
});

-- 使用
await prisma.$replica().users.findMany(); // 从库读取
await prisma.users.create({ data: {} });  // 主库写入
```

**缓存策略**

```typescript
// 多级缓存：本地缓存 + Redis
@Injectable()
export class CacheService {
  private localCache = new Map(); // 本地内存缓存

  async get(key: string) {
    // 1. 先查本地缓存（最快）
    if (this.localCache.has(key)) {
      return this.localCache.get(key);
    }

    // 2. 再查 Redis（次快）
    const redisValue = await this.redis.get(key);
    if (redisValue) {
      this.localCache.set(key, redisValue);
      return redisValue;
    }

    // 3. 最后查数据库（最慢）
    const dbValue = await this.db.get(key);
    if (dbValue) {
      await this.redis.set(key, dbValue, 'EX', 3600);
      this.localCache.set(key, dbValue);
    }

    return dbValue;
  }
}

// 缓存预热：在服务启动时加载热点数据
async onModuleInit() {
  const hotSubjects = await this.prisma.subjects.findMany();
  for (const subject of hotSubjects) {
    await this.cacheService.set(`subject:${subject.id}`, subject);
  }
}
```

---

## 8. 性能与扩展性

### 8.1 不同规模的服务器配置

#### 小规模：100-500 人在线

**月成本：约 ¥1,000**

```yaml
应用服务器: 2核4G × 1台 (¥300/月)
数据库: RDS 4G (¥400/月)
Redis: 1G (¥100/月)
OSS: 50GB (¥50/月)
CDN: 500GB (¥150/月)

性能指标:
  - QPS: 50-200
  - 响应时间: <300ms
  - 并发用户: 100-500
```

#### 中规模：500-1000 人在线

**月成本：约 ¥2,000**

```yaml
应用服务器: 4核8G × 1台 (¥500/月)
数据库: RDS 8G (¥800/月)
Redis: 2G (¥200/月)
OSS: 100GB (¥100/月)
CDN: 1TB (¥300/月)
负载均衡: SLB (¥100/月)

性能指标:
  - QPS: 200-500
  - 响应时间: <200ms
  - 并发用户: 500-1000
```

#### 中大规模：1000-5000 人在线

**月成本：约 ¥3,500**

```yaml
应用服务器: 4核8G × 2台 + 负载均衡 (¥1,200/月)
数据库: RDS 16G (1主1从) (¥2,000/月)
Redis: 4G (¥300/月)
OSS: 200GB (¥200/月)
CDN: 2TB (¥600/月)
消息队列: RabbitMQ (¥200/月)

性能指标:
  - QPS: 500-2000
  - 响应时间: <200ms
  - 并发用户: 1000-5000
```

#### 大规模：5000-20000 人在线

**月成本：约 ¥15,000**

```yaml
应用服务器: 8核16G × 4台 (¥4,000/月)
负载均衡: SLB HA (¥500/月)
数据库: RDS 32G (1主2从) (¥6,000/月)
Redis: 8G 主从 (¥800/月)
Elasticsearch: 4核8G (¥800/月)
OSS: 1TB (¥500/月)
CDN: 10TB (¥2,000/月)
消息队列: RabbitMQ Cluster (¥600/月)
监控: Prometheus + Grafana (¥300/月)

性能指标:
  - QPS: 2000-10000
  - 响应时间: <150ms
  - 并发用户: 5000-20000
```

---

### 8.2 性能优化检查清单

#### 后端优化

- [ ] **数据库优化**
  - [ ] 添加必要索引
  - [ ] 慢查询优化（<50ms）
  - [ ] N+1 查询优化
  - [ ] 连接池配置优化
  - [ ] 读写分离（大规模）

- [ ] **缓存优化**
  - [ ] 热点数据缓存（Redis）
  - [ ] 缓存命中率 >90%
  - [ ] 缓存预热
  - [ ] 缓存失效策略

- [ ] **代码优化**
  - [ ] 异步处理（消息队列）
  - [ ] 批量操作（减少数据库查询）
  - [ ] 减少不必要的计算
  - [ ] 使用连接池

#### 前端优化

- [ ] **加载优化**
  - [ ] 代码分割（React.lazy）
  - [ ] 路由懒加载
  - [ ] 图片懒加载
  - [ ] 组件懒加载
  - [ ] Bundle 大小 <500KB

- [ ] **渲染优化**
  - [ ] 虚拟列表（长列表）
  - [ ] React.memo（减少重渲染）
  - [ ] useMemo / useCallback
  - [ ] 防抖/节流

- [ ] **网络优化**
  - [ ] API 请求合并
  - [ ] 数据预取
  - [ ] CDN 加速
  - [ ] HTTP/2

---

### 8.3 扩容策略

```yaml
启动期 (0-1000 用户):
  配置: 2核4G + RDS 4G
  成本: ¥1,000/月
  策略: 单体架构，快速迭代

成长期 (1000-5000 用户):
  配置: 4核8G×2 + RDS 16G (1主1从)
  成本: ¥3,500/月
  策略: 读写分离，负载均衡
  触发条件: CPU >70%, 响应时间 >300ms

爆发期 (5000-20000 用户):
  配置: 8核16G×4 + RDS 32G (1主2从)
  成本: ¥15,000/月
  策略: 微服务化，分库分表
  触发条件: QPS >2000, 数据库连接数 >200

成熟期 (20000+ 用户):
  配置: K8s 集群 + 多地域部署
  成本: ¥50,000+/月
  策略: 全面云原生，弹性扩缩容
  触发条件: 用户量持续增长
```

---

## 9. 成本预算

### 9.1 开发成本（MVP + V1.0，4个月）

#### 人力成本

| 角色         | 人数    | 月薪（元） | 4个月成本（元）  |
| ------------ | ------- | ---------- | ---------------- |
| 产品经理     | 1       | 15,000     | 60,000           |
| 前端工程师   | 2       | 18,000     | 144,000          |
| 后端工程师   | 2       | 20,000     | 160,000          |
| UI/UX 设计师 | 1       | 12,000     | 48,000           |
| 测试工程师   | 1       | 12,000     | 48,000           |
| 运维工程师   | 1       | 15,000     | 30,000 (后2个月) |
| **合计**     | **8人** | -          | **490,000**      |

#### 服务器成本（4个月）

| 服务       | 配置   | 月成本（元） | 4个月成本（元） |
| ---------- | ------ | ------------ | --------------- |
| 应用服务器 | 4核8G  | 500          | 2,000           |
| 数据库     | RDS 8G | 800          | 3,200           |
| Redis      | 2G     | 200          | 800             |
| OSS        | 100GB  | 100          | 400             |
| CDN        | 1TB    | 300          | 1,200           |
| 域名+SSL   | -      | 100          | 400             |
| **合计**   | -      | **2,000**    | **8,000**       |

#### 其他成本

| 项目                      | 成本（元） |
| ------------------------- | ---------- |
| 第三方服务（短信、OSS等） | 5,000      |
| 设计素材/工具             | 3,000      |
| 项目管理工具              | 2,000      |
| **合计**                  | **10,000** |

#### 总成本

```
人力成本: ¥490,000
服务器成本: ¥8,000
其他成本: ¥10,000
-----------------------------
总计: ¥508,000 (约 50 万)
```

---

### 9.2 运营成本（每月）

#### 小规模（500 人）

```
服务器: ¥2,000/月
客服人员: ¥8,000/月（1人兼职）
内容运营: ¥10,000/月（1人）
-----------------------------
合计: ¥20,000/月
```

#### 中规模（1000-5000 人）

```
服务器: ¥3,500/月
技术维护: ¥20,000/月（1人）
客服团队: ¥24,000/月（3人）
内容运营: ¥30,000/月（2人）
市场推广: ¥50,000/月
-----------------------------
合计: ¥127,500/月
```

#### 大规模（5000+ 人）

```
服务器: ¥15,000/月
技术团队: ¥80,000/月（4人）
客服团队: ¥40,000/月（5人）
运营团队: ¥60,000/月（4人）
市场推广: ¥150,000/月
-----------------------------
合计: ¥345,000/月
```

---

### 9.3 成本优化建议

#### 开发期优化

1. **使用开源技术栈**（减少授权费用）
2. **云服务按量计费**（初期低成本）
3. **外包设计**（节省全职设计师成本）
4. **远程办公**（节省办公场地成本）

#### 运营期优化

1. **提升缓存命中率**（减少数据库压力）
2. **CDN 优化**（减少带宽成本）
3. **冷数据归档**（节省存储成本）
4. **预留实例**（长期使用可享 5-7 折）
5. **自动扩缩容**（非高峰期降低配置）

---

## 10. 质量保证

### 10.1 测试策略

#### 单元测试

```typescript
// 目标：代码覆盖率 >80%

// 示例：用户注册测试
describe('UserService', () => {
  describe('register', () => {
    it('should create a new user', async () => {
      const dto = {
        email: 'test@example.com',
        password: 'Password123!',
        username: 'testuser'
      };

      const user = await userService.register(dto);

      expect(user).toBeDefined();
      expect(user.email).toBe(dto.email);
      expect(user.password).not.toBe(dto.password); // 密码已加密
    });

    it('should throw error if email exists', async () => {
      const dto = { email: 'existing@example.com', ... };

      await expect(userService.register(dto))
        .rejects.toThrow('邮箱已存在');
    });
  });
});
```

#### 集成测试

```typescript
// 测试 API 接口
describe('POST /api/auth/register', () => {
  it('should register a new user', async () => {
    const response = await request(app.getHttpServer())
      .post('/api/auth/register')
      .send({
        email: 'test@example.com',
        password: 'Password123!',
        username: 'testuser',
      })
      .expect(201)

    expect(response.body.success).toBe(true)
    expect(response.body.data.user).toBeDefined()
  })
})
```

#### E2E 测试

```typescript
// 使用 Playwright 测试用户流程
test('user can register and login', async ({ page }) => {
  // 1. 访问注册页面
  await page.goto('/register')

  // 2. 填写注册表单
  await page.fill('[name="email"]', 'test@example.com')
  await page.fill('[name="password"]', 'Password123!')
  await page.click('button[type="submit"]')

  // 3. 验证注册成功
  await expect(page).toHaveURL('/dashboard')

  // 4. 退出登录
  await page.click('[data-testid="logout"]')

  // 5. 重新登录
  await page.goto('/login')
  await page.fill('[name="email"]', 'test@example.com')
  await page.fill('[name="password"]', 'Password123!')
  await page.click('button[type="submit"]')

  // 6. 验证登录成功
  await expect(page).toHaveURL('/dashboard')
})
```

#### 性能测试

```yaml
# 使用 K6 进行负载测试
import http from 'k6/http';
import { check, sleep } from 'k6';

export const options = {
  stages: [
    { duration: '2m', target: 100 },  # 2分钟爬坡到100用户
    { duration: '5m', target: 100 },  # 保持100用户5分钟
    { duration: '2m', target: 200 },  # 爬坡到200用户
    { duration: '5m', target: 200 },  # 保持200用户5分钟
    { duration: '2m', target: 0 },    # 降到0
  ],
  thresholds: {
    http_req_duration: ['p(95)<500'], # 95%的请求<500ms
  },
};

export default function () {
  const res = http.get('https://api.example.com/api/subjects');
  check(res, {
    'status is 200': (r) => r.status === 200,
    'response time < 500ms': (r) => r.timings.duration < 500,
  });
  sleep(1);
}
```

---

### 10.2 代码质量检查清单

#### 代码规范

- [ ] ESLint 配置并通过检查
- [ ] Prettier 格式化
- [ ] TypeScript 严格模式
- [ ] Commit 规范（Conventional Commits）
- [ ] 代码审查（Code Review）

#### 安全检查

- [ ] 依赖包漏洞扫描（npm audit）
- [ ] 敏感信息检测（环境变量）
- [ ] SQL 注入防护测试
- [ ] XSS 攻击防护测试
- [ ] CSRF 攻击防护测试
- [ ] API 限流配置

#### 性能检查

- [ ] API 响应时间 <200ms
- [ ] 页面首屏加载 <3s
- [ ] Bundle 大小 <500KB
- [ ] 数据库慢查询 <50ms
- [ ] 缓存命中率 >90%

---

### 10.3 上线检查清单

#### 功能完整性

- [ ] 所有核心功能测试通过
- [ ] 边界情况测试通过
- [ ] 异常情况处理正确
- [ ] 移动端适配完整

#### 性能指标

- [ ] 负载测试通过（1000+ 并发）
- [ ] 压力测试通过
- [ ] API 响应时间达标
- [ ] 数据库性能达标

#### 安全性

- [ ] 渗透测试通过
- [ ] 漏洞扫描通过
- [ ] SSL 证书配置正确
- [ ] 备份策略配置完成

#### 监控告警

- [ ] Sentry 错误监控配置
- [ ] Google Analytics 配置
- [ ] 服务器监控配置
- [ ] 告警规则配置

#### 文档准备

- [ ] API 文档完整
- [ ] 用户手册准备
- [ ] 运维文档准备
- [ ] 应急预案准备

---

## 11. 附录

### 11.1 开发规范

#### Git Commit 规范

```
<type>(<scope>): <subject>

type:
  - feat: 新功能
  - fix: 修复 bug
  - docs: 文档更新
  - style: 代码格式调整
  - refactor: 重构
  - test: 测试相关
  - chore: 构建/工具相关

示例:
  feat(auth): 添加验证码登录功能
  fix(course): 修复视频播放进度不同步的问题
  docs(api): 更新 API 文档
```

#### 代码审查检查项

- [ ] 代码符合项目规范
- [ ] 没有明显的 bug
- [ ] 逻辑清晰易懂
- [ ] 没有安全隐患
- [ ] 性能没有明显问题
- [ ] 测试用例完整
- [ ] 注释清晰（复杂逻辑）

---

### 11.2 参考资源

#### 官方文档

- React: https://react.dev
- NestJS: https://nestjs.com
- Prisma: https://www.prisma.io
- shadcn/ui: https://ui.shadcn.com
- PostgreSQL: https://www.postgresql.org

#### 技术博客

- 阿里云技术博客
- 腾讯云开发者社区
- InfoQ 中文站

#### 学习资源

- NestJS 中文文档
- React 官方教程
- Prisma 实战指南

---

### 11.3 常见问题 FAQ

**Q1: 为什么选择 NestJS 而不是 Express？**
A: NestJS 提供了更好的模块化设计、依赖注入、TypeScript 原生支持，适合构建企业级应用。Express 更适合小型项目。

**Q2: 是否需要使用 Docker？**
A: 推荐使用。Docker 可以保证开发、测试、生产环境一致，简化部署流程。

**Q3: 数据库选 MySQL 还是 PostgreSQL？**
A: 推荐 PostgreSQL，功能更强大（支持 JSON、全文搜索等），性能也不错。MySQL 也是不错的选择。

**Q4: 前端状态管理选 Redux 还是 Zustand？**
A: 小中型项目推荐 Zustand（简单轻量），大型项目可以考虑 Redux Toolkit。

**Q5: 什么时候需要微服务化？**
A: 当用户量超过 10000，单体应用性能出现瓶颈时再考虑微服务化。过早微服务化会增加复杂度。

---

## 12. 总结

本文档详细规划了中学生在线教育平台从 0 到 1 的完整开发路径，包括：

✅ **产品功能设计**：6 大核心模块，25+ 数据表
✅ **技术架构设计**：前后端分离，微服务化预留
✅ **开发路线图**：MVP（2个月）→ V1.0（2个月）→ V2.0（持续迭代）
✅ **技术难点攻克**：学习进度同步、答题批改、排行榜、视频防盗链、高并发
✅ **性能与扩展**：从 500 到 20000+ 并发的扩容方案
✅ **成本预算**：开发成本 50 万，运营成本 2-15 万/月
✅ **质量保证**：测试策略、代码规范、上线检查清单

---

## 📞 联系方式

如有疑问，请联系项目负责人。

---

**文档版本**：v1.0
**最后更新**：2025-01-08
**作者**：产品团队

---

_本文档为内部开发指南，请勿外传。_
