      ...
      <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8`}>
        ...
        `}>
          ...
          <div className="hidden md:flex items-center space-x-3">
            ...
            <Button size="sm" variant="glow" onClick={() => navigate('/register')} className="shadow-lg shadow-blue-500/20">
              {text.start}
            </Button>
          </div>
        ...
        <div className="absolute top-full left-0 right-0 mt-2 px-4 md:hidden animate-fade-in-up z-50">
          <div className="p-4 space-y-2 bg-[#0f0f0f]/95 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl">
            ...
            <div className="flex flex-col gap-3">
               <Button variant="outline" fullWidth onClick={() => { setIsMobileMenuOpen(false); navigate('/login'); }}>{text.login}</Button>
               <Button variant="glow" fullWidth onClick={() => { setIsMobileMenuOpen(false); navigate('/register'); }}>{text.join}</Button>
            </div>
          </div>
        </div>
      )}
