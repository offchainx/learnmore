...
- [ ] 创建 `leaderboard_entries` 数据库表
- [ ] 实现 `updateLeaderboardScore()` Server Action (更新分数)
- [ ] 实现 `getLeaderboard()` 查询函数 (获取排行榜数据)
- [ ] 前端展示排行榜页面 `/leaderboard` (Top 100 + 自己的排名)
- [ ] 实现周榜/月榜切换功能
- [ ] 添加性能索引,确保查询速度 < 200ms
- [ ] 预留 Redis 迁移接口 (通过 Adapter Pattern)
