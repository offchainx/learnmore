...
- [x] 运行 `pnpm dev`,应用在 `http://localhost:3000` 启动成功
- [x] 访问首页,无控制台错误
- [x] 页面显示 Shadcn 的 Button 组件,且样式正确 (Tailwind 生效)
- [x] 控制台无 Hydration Error

### 代码质量测试

- [x] 运行 `pnpm lint`,无 ESLint 错误
- [x] 运行 `pnpm build`,构建成功
- [x] Git 状态干净 (除 node_modules, .next, .env.local 外)

### 环境变量测试

- [x] 创建测试文件验证 Supabase 连接:
...
- [x] Prettier 格式化完成 (运行 `pnpm format`)
- [x] 无 TypeScript 类型错误 (`pnpm tsc --noEmit`)
...
- [x] README.md 包含:
  ...
  - 本地开发启动步骤
  - 环境变量说明
- [x] `.env.example` 文件已创建并包含所有必需变量

### 部署就绪

- [x] Git 仓库已初始化 (`.gitignore` 配置正确)
- [x] 首次 Commit 已提交并 Push
- [ ] 可选: Vercel 项目已创建并关联 (为后续自动部署准备)

### 团队协作

- [x] 在项目看板中标记此 Story 为"已完成"
- [x] 通知团队成员: "开发环境已就绪,可以开始 Story-002"
