...
| `/dashboard/practice/import` | 练习题导入 | `src/app/(dashboard)/dashboard/practice/import/page.tsx` | 🚧 智能题目解析和导入 (开发中) |
| `/dashboard/practice/debug` | 练习系统调试 | `src/app/(dashboard)/dashboard/practice/debug/page.tsx` | 🚧 开发调试工具 (开发中) |
...
| 路由 | 页面名称 | 文件路径 | 功能说明 |
|------|---------|---------|---------|
| `/error-book` | 错题本 | `src/app/error-book/page.tsx` | 错题回顾、掌握度追踪 |
...
当前正在开发的路由 (Story-010: Practice Center Revamp):

1. **`/dashboard/practice/import`**
   - 智能题目解析器
   - 支持图片上传和OCR识别
   - 使用 Gemini AI 自动提取题目信息

2. **`/dashboard/practice/debug`**
   - 开发者调试工具
   - 测试题目解析功能
   - 查看解析结果和错误日志
