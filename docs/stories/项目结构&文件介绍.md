...
#### 当前项目页面列表 (基于 `src/app` 目录结构)

1.  **营销与公共页面 (Marketing & Public Pages)**
    *   **主页 (Landing Page)**: `/` (`src/app/page.tsx`)
    *   **工作原理 (How It Works)**: `/how-it-works` (`src/app/how-it-works/page.tsx`)
    *   **课程体系 (Subjects)**: `/subjects` (`src/app/subjects/page.tsx`)
    *   **价格方案 (Pricing)**: `/pricing` (`src/app/pricing/page.tsx`)
    *   **关于我们 (About Us)**: `/about-us` (`src/app/about-us/page.tsx`)
    *   **成功案例 (Success Stories)**: `/success-stories` (`src/app/success-stories/page.tsx`)
    *   **博客/资讯 (Blog)**: `/blog` (`src/app/blog/page.tsx`)
    *   **学习指南 (Study Guides)**: `/study-guides` (`src/app/study-guides/page.tsx`)
    *   **学生关怀 (Student Care)**: `/student-care` (`src/app/student-care/page.tsx`)
    *   **联系我们 (Contact)**: `/contact` (`src/app/contact/page.tsx`)
    *   **隐私政策 (Privacy)**: `/privacy` (`src/app/privacy/page.tsx`)
    *   **服务条款 (Terms)**: `/terms` (`src/app/terms/page.tsx`)
    *   **未找到页面 (Not Found)**: `/_not-found` (Next.js 默认)

2.  **认证相关页面 (Authentication Pages)**
    *   **登录 (Login)**: `/login` (`src/app/(auth)/login/page.tsx`)
    *   **注册 (Register)**: `/register` (`src/app/(auth)/register/page.tsx`)

3.  **用户仪表盘页面 (User Dashboard Pages)**
    *   **仪表盘主页 (Dashboard Home)**: `/dashboard` (`src/app/(dashboard)/dashboard/page.tsx`)
    *   **社区 (Community)**: `/dashboard/community` (`src/app/(dashboard)/community/page.tsx`)
    *   **社区帖子详情 (Community Post Detail)**: `/dashboard/community/[postId]` (`src/app/(dashboard)/community/[postId]/page.tsx`)
    *   **创建社区帖子 (Create Community Post)**: `/dashboard/community/new` (`src/app/(dashboard)/community/new/page.tsx`)
    *   **排行榜 (Leaderboard)**: `/dashboard/leaderboard` (`src/app/(dashboard)/leaderboard/page.tsx`)
    *   **设置 (Settings)**: `/dashboard/settings` (`src/app/(dashboard)/settings/page.tsx`)

4.  **课程相关页面 (Course Pages)**
    *   **课程详情 (Course Detail)**: `/course/[subjectId]` (`src/app/course/[subjectId]/page.tsx`)
    *   **课程学习 (Lesson View)**: `/course/[subjectId]/[lessonId]` (`src/app/course/[subjectId]/[lessonId]/page.tsx`)

5.  **错题本页面 (Error Book Page)**
    *   **错题本 (Error Book)**: `/error-book` (`src/app/error-book/page.tsx`)
