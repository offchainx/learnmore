...
- 本轮结论：
  ...
  - `T-020.10` 已完成边界与故障场景验证，重复点击、重复提交、越权、会话失效、脏参数、网络失败、服务异常与局部失败降级都已被收口。

### T-020.10 边界与故障场景验证（已完成）
- 本轮目标是把 `T-020.6 ~ T-020.9` 已验证的主链路，进一步压到重复提交、异常返回和局部降级这些真实故障边界上，避免主流程“能跑通但不稳”。
- 已收口的关键边界包括：
  - 练习副作用链路改为容错执行，`applyPracticeSubmissionEffects()` 不再因为缓存失效或个别 side effect 失败而让主提交失败。
  - 成就 / streak 的缓存回收改为 `try/catch` 包裹，`revalidateTag` 异常只记录告警，不污染写入结果。
  - `claimTaskReward()` 与 `completeOnboardingTask()` 对未登录统一返回结构化错误，不再直接抛异常；前端 `DailyMissions` 也补上失败提示，避免静默吞错。
- 已同步补齐的代码与测试：
  - [`src/actions/practice/submission-effects.ts`](/Users/victorsim/Desktop/Projects/learn_more_v1.0/src/actions/practice/submission-effects.ts)
  - [`src/actions/gamification/streak.ts`](/Users/victorsim/Desktop/Projects/learn_more_v1.0/src/actions/gamification/streak.ts)
  - [`src/actions/gamification/achievements.ts`](/Users/victorsim/Desktop/Projects/learn_more_v1.0/src/actions/gamification/achievements.ts)
  - [`src/actions/gamification/achievement.ts`](/Users/victorsim/Desktop/Projects/learn_more_v1.0/src/actions/gamification/achievement.ts)
  - [`src/components/dashboard/DailyMissions.tsx`](/Users/victorsim/Desktop/Projects/learn_more_v1.0/src/components/dashboard/DailyMissions.tsx)
  - [`src/actions/practice/__tests__/submission-effects.test.ts`](/Users/victorsim/Desktop/Projects/learn_more_v1.0/src/actions/practice/__tests__/submission-effects.test.ts)
  - [`src/actions/__tests__/achievement.test.ts`](/Users/victorsim/Desktop/Projects/learn_more_v1.0/src/actions/__tests__/achievement.test.ts)
  - [`src/actions/__tests__/streak.test.ts`](/Users/victorsim/Desktop/Projects/learn_more_v1.0/src/actions/__tests__/streak.test.ts)
- 已执行针对性回归：
  - `pnpm vitest src/actions/__tests__/achievement.test.ts src/actions/__tests__/streak.test.ts src/actions/practice/__tests__/submission-effects.test.ts`
  - 结果为 `8 passed`
- 本轮结论：
  - 主提交链路现在不会再被缓存失效或局部副作用失败放大成整体失败；
