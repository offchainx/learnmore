...
| T-006.24 | 设定页补齐：浅色主题与语言设定两个问题 | codex | doing | 2026-03-19：已修复 Settings 中语言/主题偏好草稿与提交链路，`system` 主题不再误切 light/dark，并移除误导性的“即时生效”文案；但全站浅色主题页面覆盖仍未过验收，继续保留 doing。 |
| T-006.24.1 | 浅色主题 token 合同：补齐 page / surface / text / border / shadow / state 语义层，建立浅色模式基础令牌映射 | codex | doing | 2026-03-13：已完成第一轮 token / ThemeProvider 接线；但 2026-03-13 下午截图验收显示页面内容层仍大量保留深色硬编码，需返工 token 覆盖与浅色对比度。 |
| T-006.24.2 | 全局壳层与导航适配浅色主题：sidebar、topbar、page shell、hero 容器的浅色层级与选中态规则统一 | codex | doing | 2026-03-13：第一轮已完成 `DashboardLayout` 与 `PageHeroShell` 接线；但截图验收显示 sidebar/page shell 已浅色化，主内容 hero 与大量页面容器仍是深色舱体，未通过页面级验收。 |
...
| T-006.24.4 | 数据型组件适配浅色主题：KPI 卡、表格、筛选工具栏、状态 badge、空态模板在浅色模式下统一 | codex | doing | 2026-03-13：第一轮已完成共享 `Card / Table` 与 `content-reports` 接线；但截图验收显示后台页覆盖不均，仍有大量 KPI、工作区、工具栏与空态使用自定义深色容器，需返工统一。 |
| T-006.24.5 | 页面例外项收口：Dashboard / 课程 / 练习中心 / 社区 / 定价 / 排行榜 / 成就中心 / 设置页的浅色特例逐页收口 | codex | doing | 2026-03-14：开始逐页清理仍绕过 token 的深色私有壳层，优先处理 Pricing / Leaderboard / Achievements，并回补 Dashboard / Courses / Practice / Community / Settings 残留例外。2026-03-14：已完成 Leaderboard 实际渲染组件（TierRoadmap / LeaderboardList / XPBreakdown / FocusPanel）与 Achievements 徽章墙的第一轮浅色收口；Pricing 改版已回滚，后续需按营销页单独重做。 |
| T-006.24.6 | 浅色主题回归矩阵：hover / focus / active / disabled / empty / loading / selected 状态逐项验证 | codex | doing | 2026-03-14：按 `Settings / Leaderboard / Achievements / Dashboard / Courses / Practice / Community / Admin` 八页矩阵推进；第一轮先验证 `hover / focus / selected / disabled / empty / loading` 六类状态，优先处理 Settings + Leaderboard + Achievements。2026-03-14：第一轮已补齐 Settings 语言/外观选择组与表单输入的 focus/disabled 基线，补齐 Leaderboard 与 Achievements 自定义 pills 的 focus 态。2026-03-15：根据截图验收继续收口 Settings 的误导性“即时生效”提示、Leaderboard 的当前项高亮与状态统一、Achievements 的锁定卡对比度与少量结果布局；当前矩阵仍暂记 `Settings=partial`、`Leaderboard=partial`、`Achievements=partial`，待下一轮截图复核后再决定是否 accepted。2026-03-19：代码扫描补充矩阵，当前暂记 `Dashboard=partial`、`Courses=partial`、`Practice=partial`、`Community=partial`、`Settings=partial`、`Pricing=partial`、`Leaderboard=partial`、`Achievements=partial`、`Admin=partial`；主要阻塞落在 `Widgets / DailyMissions / LessonPlayer / PracticeModePreviewDialog / SmartDrillPreviewDialog / chapter-drill/* / RivalWatch / QuestionReviewTable / admin users tabs` 等残留深色私有壳层。 |
