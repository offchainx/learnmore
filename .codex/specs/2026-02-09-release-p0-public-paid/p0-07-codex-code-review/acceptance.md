...
| Vercel 慢响应诊断（T-009） | `curl -w` 对比 `learnmorev10.vercel.app` / preview / localhost + 响应头分析 | 明确瓶颈并给出修复 | pass | 修复前：生产首页 TTFB 约 2.70~2.93s，且 `x-vercel-cache: MISS` + `cache-control: private,no-store`，`x-vercel-id` 显示 `sin1::iad1`（跨区执行）。修复后（2026-03-09 17:06+ redeploy）：`learnmorev10.vercel.app` 首次约 1.72s，后续稳定约 0.37~0.45s；响应头为 `x-vercel-cache: HIT`、`x-nextjs-prerender: 1`、`x-nextjs-stale-time: 300`，执行节点为 `sin1`。已落地 `preferredRegion='sin1'` + stats 缓存 + 移除首页服务端 `getSession()` + `dynamic='force-static'` + `revalidate=300`。 |
